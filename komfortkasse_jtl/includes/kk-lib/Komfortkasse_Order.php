...
class Komfortkasse_Order {
	...
	public static function getOpenIDs($type = 'cBestellNr') {
		...
		if (Komfortkasse_Config::getConfig ( Komfortkasse_Config::activate_export )) {
			...
			$rows = \JTL\Shop::Container()->getDB()->query($query, 2);
	...
	public static function getOrder($number, $type = 'cBestellNr') {
		if ($type == 'cBestellNr') {
		    $obj = \JTL\Shop::Container()->getDB()->select('tbestellung', 'cBestellNr', $number);
			$number = $obj->kBestellung;
		}
		$oBestellung = new \JTL\Checkout\Bestellung ( $number );
		...
		$oSprache = \JTL\Shop::Container()->getDB()->select('tsprache', 'ksprache', $oBestellung->kSprache);
		...
		foreach ( $oBestellung->Positionen as $item ) {
			if ($item->kArtikel > 0) {
				$art = new \JTL\Catalog\Product\Artikel();
	...
	public static function updateOrder($order, $status, $callbackid) {
		...
		$obj = \JTL\Shop::Container()->getDB()->select('tbestellung', 'cBestellNr', $order['number']);
		$oBestellung = new \JTL\Checkout\Bestellung ( $obj->kBestellung );
		$oBestellung->fuelleBestellung ( 1, 0, false );
		...
		if ($status == BESTELLUNG_STATUS_BEZAHLT) {
			...
			\JTL\Shop::Container()->getDB()->insert('tzahlungseingang', $zahlungseingang);
